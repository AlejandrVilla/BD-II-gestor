...
{
public:
  BPTreeElement(string _fileName, int _id, int _length);                           // constructor
  BPTreeElement(string _fileName, int _id, int _length, bool _leaf, int firstPtr); // constructor

  ~BPTreeElement(); // destructor

  int getSize() const;                                                               // obtener tamaño de nodo
  int getKeyLength() const;                                                          // obtener el tamaño de la llave
  bool isLeaf() const;                                                               // si el nodo es hoja
  unsigned char *getKey(int pos) const;                                              // obtener llave
  int getPointer(int pos) const;                                                     // obtener puntero
  int findPosition(unsigned char *key) const;                                        // encontrar la posicion de la llave
  void setKey(int pos, unsigned char *key);                                          // almacenar la posición de la llave
  void setPointer(int pos, int ptr);                                                 // alamcenar puntero de posición
  void setRemoved();                                                                 // alamcenar bloque removid
  void insert(int pos, unsigned char *key, int ptr);                                 // Insert key-pointer after position
  void remove(int pos);                                                              // Remove key-pointer at position
  BPTreeElement *split(int newId, unsigned char *newKey);                            // Split into two nodes. Return new node
  unsigned char *borrow(BPTreeElement *sib, bool leftSib, unsigned char *parentKey); // Borrow a key from sibling. Return new parent key
  void mergeRight(BPTreeElement *sib, unsigned char *parentKey);                     // Merge right sibling

private:
  string fileName;              // nombre del nodo
  int id;                       // bloque id en el file
  int size;                     // tamaño del nodo
  int length;                   // tamaño de cada llave
  bool leaf;                    // si el nodo es una hoja
  bool dirty;                   // si el nodo ha sido modificado
  bool blockRemoved;            // si el bloque ha sido removido
  vector<unsigned char *> keys; // laves nodo
  vector<int> ptrs;             // punteros de nodo
};

// Constructor from file
BPTreeElement::BPTreeElement(string _fileName, int _id, int _length) : fileName(_fileName), id(_id), length(_length)
{
