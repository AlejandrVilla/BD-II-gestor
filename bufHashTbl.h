...
namespace DB
{
    ...
    void BufHashTbl::insert(const File *file, const int pageNo, const int frameNo)
    {
    ...
    void BufHashTbl::lookup(const File *file, const int pageNo, int &frameNo)
    {
        int index = hash(file, pageNo);
        hashBucket *tmpBuc = ht[index];
        while (tmpBuc)
    ...
    void BufHashTbl::remove(const File *file, const int pageNo)
    {
